...
class GasCSTRReactor:
    ...
    def _build_dT_dt(
        ...
        Energy balance
        --------------
        dT/dt = (En_in - En_out + Q_rxn + Q_exchange + Q_constant) / (Σ_i n_i Cp_i)
        """
        ...
        # NOTE: En_IG(T) for outlet stream components [J/mol]
        _, En_IG_values_out = self.thermo_source.calc_En_IG(
            temperature=temperature
        )

        # Fi[mol/s], En_IG[J/mol] => En_flow [W]
        En = np.sum(self.F_in * (self.En_IG_values_in - En_IG_values_out))
