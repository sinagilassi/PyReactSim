...
class LiquidBatchReactor(ReactorAuxiliary, ReactLog):
    ...
    def rhs(
        ...
        # NOTE: Calculate Reaction rates for each component (concentration and temperature)
    ...
    def _build_dT_dt(
        ...
        # ! calculate volumetric heat capacity of liquid mixture (Σ_i c_i Cp_i)
        # J/m3.K
        Cp_LIQ_MIX_VOLUMETRIC = self._calc_volumetric_heat_capacity(
            c=c,
            temperature=temperature,
            mode=cast(
                Literal['constant', 'calculate'],
