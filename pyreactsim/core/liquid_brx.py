...
class LiquidBatchReactorX(LiquidBatchReactor):
    """
    Scaled liquid-phase batch reactor model.

    The underlying physical equations are identical to LiquidBatchReactor. Only
    the state presented to the ODE solver is scaled.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        # Species scaling uses initial moles with a protective floor.
        self.N_scale = np.maximum(self.N0.astype(float), 1e-8)

        # Temperature scaling uses deviation form.
