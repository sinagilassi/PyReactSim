...
class LiquidPBRReactorX(LiquidPBRReactor):
    """
    Scaled liquid-phase packed-bed reactor model.

    Physical balances are inherited from LiquidPBRReactor; only solver-state
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Species scaling from inlet molar flows with a protective floor.
        self.F_scale = np.maximum(self.F_in.astype(float), 1e-8)

        # Temperature scaling as deviation around inlet temperature.
